...
| S3 对象存储 | [`framework-storage-s3-starter`](https://github.com/describeadmin/framework-storage-s3-starter) | `StorageProvider` | **0.2.0** | 已发布 v0.2.0 | 第一个存储层插件。把 `StorageProvider` 从框架自带的 `LocalFileStorageProvider` 切到 S3 / S3 兼容对象存储（AWS S3、MinIO、RustFS）。基于 AWS SDK for Java v2（`software.amazon.awssdk:s3`），固定 `url-connection-client` 为唯一 HTTP 实现并排除 `apache5-client`/`apache-client`/`netty-nio-client`（两个实现同时在 classpath 上，SDK 建 client 时抛 "Multiple HTTP implementations found"）。装配 `before = FrameworkStorageAutoConfiguration.class`（硬依赖直接引用类，无 optional 依赖故无 `beforeName` 字符串形式），全部 Bean `@ConditionalOnMissingBean`（业务方自注册的 `S3Client`/`StorageProvider` 一定赢）。运行时开关 `describeadmin.storage.s3.enabled`，关掉后连 `S3Client` 都不建、退回本地磁盘实现。`presignedUrl()` 返回真实签名地址（私有桶下 `url()` 通常不可访问）。客户端校验和策略退回 `WHEN_REQUIRED`（AWS SDK 2.30 起默认发 `x-amz-checksum-crc32` 而非 `Content-MD5`，老 S3 兼容实现不认会 400）。与本地实现唯一有意的行为差异：`put` 的 `size>0` 时被当作真实 `Content-Length`，对不上是调用方错误（S3 流式上传的固有约束）。27 个测试全绿（装配顺序 `ApplicationContextRunner` + 端到端读写 / 预签名 URL 真实 HTTP 下载 / key 校验 / 键前缀 / auto-create-bucket），Testcontainers MinIO 与外部 RustFS 两个后端均验证 |
...
> 上表前四个插件（cache-redis / auth-email / crypto / excel）均于 **2026-08-31** 随
> framework 0.2.0 同批发布到 Maven Central，坐标 `io.github.describeadmin:framework-<能力>-starter:0.2.0`。
> `framework-storage-s3-starter` 于 **2026-09-01** 单独建仓并发布 `0.2.0`（tag `v0.2.0` 触发
> release 工作流，制品已上传 Central Portal draft，按 `RELEASE.md` §4.2 d 手动 Publish）。
...
> `framework-parent` 的 `<dependencies>` 给每个模块白送了 `spring-boot-starter-test`，
> 独立成仓后必须自己声明。漏了要到编译测试时才报错，而报错信息指向的是
> "找不到 JUnit"，不指向拆仓这件事。
>
> lombok 不在此列——框架源码不用它（只有 codegen 生成物用，那份依赖由 archetype
> 写进业务方 pom）。插件源码如确需 lombok，自己声明 `org.projectlombok:lombok`
> （`optional`，不写版本，由 import 进来的 `framework-bom → spring-boot-dependencies` 仲裁）。
